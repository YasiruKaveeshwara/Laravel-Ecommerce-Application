...
const STATUS_META: Record<
  string,
  { label: string; badge: string; description: string; accent: string; icon: ComponentType<{ className?: string }> }
> = {
  processing: {
    label: "Processing",
    badge: "border-amber-200 bg-amber-50 text-amber-700",
    description: "We are confirming payment details and preparing your products.",
    accent: "from-amber-500/20 via-amber-500/30 to-orange-500/20",
    icon: PackageCheck,
  },
  shipped: {
    label: "Shipped",
    ...
    accent: "from-sky-500/20 via-sky-600/30 to-slate-900/10",
    icon: Truck,
  },
  delivered: {
    label: "Delivered",
    ...
    accent: "from-emerald-500/20 via-emerald-500/40 to-slate-900/20",
    icon: PackageCheck,
  },
  cancelled: {
    label: "Cancelled",
    ...
    accent: "from-rose-500/20 via-rose-500/40 to-slate-900/10",
    icon: PackageCheck,
  },
};
...
const DEFAULT_STATUS = {
  ...
  accent: "from-slate-500/20 via-slate-600/40 to-slate-900/10",
  icon: PackageCheck,
};
...
export function OrderDetailView({ order, context = "customer" }: { order: Order; context?: "customer" | "admin" }) {
  ...
  return (
    <div className='space-y-6'>
      <section
        className={cn(
          "rounded-3xl border border-white/10 bg-gradient-to-br p-6 text-white shadow-2xl",
          statusMeta.accent,
          "from-10% via-30%"
        )}>
        <div className='flex flex-wrap items-start gap-4'>
          <div>
            <p className='text-xs font-semibold uppercase tracking-[0.4em] text-white/70'>Order</p>
            <h1 className='text-3xl font-semibold leading-tight'>#{orderCode}</h1>
            <p className='text-sm text-white/70'>Placed {placedOn}</p>
          </div>
          <span
            className={cn(
              "ml-auto inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-sm font-semibold",
              statusMeta.badge
            )}>
        ...
        <p className='mt-4 text-sm text-white/80'>{statusMeta.description}</p>
        <div className='mt-6 grid gap-4 md:grid-cols-3'>
          {timeline.map((step, index) => (
            <div
              key={step.label}
              className={cn(
                "rounded-2xl border border-white/15 px-4 py-3 text-sm",
                step.completed ? "bg-white/15" : "bg-white/5"
              )}>
